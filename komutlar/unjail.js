...
const cezalırol = '830363229733126163'//Jail Rol 
const jaillog = message.guild.channels.cache.find(c => c.id === '830363322976567296')//Jail Log
...
if(kullanici.id === client.user.id)return message.channel.send(new MessageEmbed().setDescription(`${message.author}, Bir botu komutu kullanamazsın.`).setImage("https://maxsoft.web.tr/guard.gif").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0x800d0d').setTimestamp()).then(x => x.delete({timeout: 5000}));
